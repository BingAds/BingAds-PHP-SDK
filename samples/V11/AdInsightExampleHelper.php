...
use Microsoft\BingAds\V11\AdInsight\GetEstimatedBidByKeywordIdsRequest;
use Microsoft\BingAds\V11\AdInsight\GetEstimatedBidByKeywordsRequest;
use Microsoft\BingAds\V11\AdInsight\GetEstimatedPositionByKeywordIdsRequest;
use Microsoft\BingAds\V11\AdInsight\GetEstimatedPositionByKeywordsRequest;
use Microsoft\BingAds\V11\AdInsight\GetHistoricalKeywordPerformanceRequest;
use Microsoft\BingAds\V11\AdInsight\GetHistoricalSearchCountRequest;
use Microsoft\BingAds\V11\AdInsight\GetKeywordCategoriesRequest;
use Microsoft\BingAds\V11\AdInsight\GetKeywordDemographicsRequest;
...
use Microsoft\BingAds\V11\AdInsight\GetKeywordLocationsRequest;
use Microsoft\BingAds\V11\AdInsight\GetKeywordOpportunitiesRequest;
use Microsoft\BingAds\V11\AdInsight\GetKeywordTrafficEstimatesRequest;
use Microsoft\BingAds\V11\AdInsight\SuggestKeywordsForUrlRequest;
...
final class AdInsightExampleHelper {
    ...
    static function OutputBidLandscapePoint($dataObject)
    {
        ...
        if (!empty($dataObject))
        {
            ...
            self::OutputStatusMessage(sprintf("Currency: %s", $dataObject->Currency));
    ...
    static function OutputKeywordDemographic($dataObject)
    {
        ...
        if (!empty($dataObject))
        {
            ...
            self::OutputStatusMessage(sprintf("Age18_24: %s", $dataObject->Age18_24));
            self::OutputStatusMessage(sprintf("Age25_34: %s", $dataObject->Age25_34));
            self::OutputStatusMessage(sprintf("Age35_49: %s", $dataObject->Age35_49));
            self::OutputStatusMessage(sprintf("Age50_64: %s", $dataObject->Age50_64));
            self::OutputStatusMessage(sprintf("Age65Plus: %s", $dataObject->Age65Plus));
    ...
    static function OutputCurrency($valueSet)
    {
        self::OutputStatusMessage("* * * Begin OutputCurrency * * *");
        self::OutputStatusMessage(sprintf("Values in %s", $valueSet->type));
        foreach ($valueSet->string as $value)
        {
            self::OutputStatusMessage($value);
        }
        self::OutputStatusMessage("* * * End OutputCurrency * * *");
    }
    static function OutputArrayOfCurrency($valueSets)
    {
        if(count((array)$valueSets) == 0)
        {
            return;
        }
        self::OutputStatusMessage("* * * Begin OutputArrayOfCurrency * * *");
        foreach ($valueSets->Currency as $valueSet)
        {
            self::OutputCurrency($valueSet);
        }
        self::OutputStatusMessage("* * * End OutputArrayOfCurrency * * *");
    }
